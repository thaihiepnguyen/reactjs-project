...
export class AuthService {
  ...
  async validateUser(email: string, pass: string): Promise<any> {
    ...
    if (!user) {
      throw new HttpException('Email not exists', HttpStatus.FORBIDDEN);
    }
    ...
    if (!isMatch) {
      throw new HttpException('Password not correct', HttpStatus.FORBIDDEN);
    }
  ...
  public async sendResetPasswordMail(
    ...
    const rawData = await this.connection.query(
      `SELECT * FROM email_templates WHERE id = 2`,
    );
    const content = rawData[0].content;
    const html = content
      .replace('$user_name$', fullname)
  ...
  public async verifyEmail(token: string): Promise<boolean> {
    ...
    try {
      ...
      await this.connection
        .getRepository(Users)
        .update({ id: payload.id }, { isActive: true });
    }
    return true;
  }
  ...
  public async register(
    ...
    const userDB = await this.userService.createUser(
      fullname,
      email,
      encryptedPassword,
    );
